...
class PatientModel {
    ...
    func parseBreathJSON(_ json: [[String: Any]]) -> ([Double], [Double], [Int], [Double], [String], [Int]) {
        ...
        for (index, breath) in json.enumerated() {
            if let temp = breath["vwd"] as? [String: [Double]], let flowSet = temp["flow"], let pressureSet = temp["pressure"], let date = (breath["breath_meta"] as? [String: Any])?["abs_bs"] as? String, let classifications = breath["classifications"] as? [String: Int] {
                flowData += flowSet
